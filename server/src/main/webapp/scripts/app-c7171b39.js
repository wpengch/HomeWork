...
(function () {
    ...
    TitleInfoCtrl.$inject = ['$log', 'Config', '$rootScope', 'Restangular', 'DialogFactory', '$mdDialog', '$stateParams'];

    function TitleInfoCtrl($log, Config, $rootScope, Restangular, DialogFactory, $mdDialog, $stateParams) {
        //接口定义
        var vm = this;
...
(function () {
    ...
    function MenuFactory($log, Config, Restangular, $q) {
        ...
        factory.getSections = function () {
            return Restangular.all('menu').getList().$object;
        };
        factory.selectPage = selectPage;
        factory.selectSection = function (section) {
            factory.openedSection = section;
        };
        factory.toggleSelectSection = function (section) {
            factory.openedSection = (factory.openedSection === section ? null : section);
        };
        factory.isSectionSelected = function (section) {
...
angular.module("home").run(["$templateCache", function ($templateCache) {
    $templateCache.put("app/course/Course.html", "<div ui-view=\"\" class=\"content-padding\"></div>");
    ...
    $templateCache.put("app/title/info/TitleInfo.html", "<div layout=\"row\"><md-whiteframe class=\"md-whiteframe-z4\" layout=\"column\" flex=\"\"><md-toolbar md-theme=\"altTheme\" layout=\"row\" class=\"toolbar-border\"><div class=\"md-toolbar-tools\" flex=\"\">{{vm.course.name}} 学生列表</div><md-button ng-click=\"vm.addStudent($event)\">添加学生</md-button></md-toolbar><div layout=\"row\" ng-repeat=\"student in vm.course.students\"><md-button flex=\"\">{{$index+1}}</md-button><md-button flex=\"\">{{student.name}}</md-button><md-button flex=\"\" ng-click=\"vm.removeStudent(student, $index, $event)\">删除</md-button></div><div ng-if=\"!vm.course.students || vm.course.students.length === 0\"><md-button ng-click=\"vm.addStudent($event)\">没有学生，点此添加</md-button></div></md-whiteframe></div>");
    $templateCache.put("app/title/list/TitleList.html", "<div layout=\"row\"><md-whiteframe class=\"md-whiteframe-z4\" layout=\"column\" flex=\"\"><md-toolbar md-theme=\"altTheme\" layout=\"row\" class=\"toolbar-border\"><div class=\"md-toolbar-tools\" flex=\"\">我的题目</div><md-button ng-href=\"/#/main/title/add\">添加</md-button></md-toolbar><div layout=\"row\" ng-repeat=\"title in vm.user.titles\"><md-button flex=\"\">{{$index+1}}</md-button><md-button flex=\"\" ng-href=\"/#/main/title/info?id={{title.id}}\">{{title.name}}</md-button><md-button flex=\"\">{{title.description}}</md-button></div><div ng-if=\"!vm.user.titles || vm.user.titles.length === 0\"><md-button ng-href=\"/#/main/title/add\">没有出过题目，点此添加</md-button></div></md-whiteframe></div>");
