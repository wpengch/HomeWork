...
$templateCache.put("app/title/add/TitleAdd.html","<div layout=\"row\"><form name=\"vm.addForm\" class=\"md-whiteframe-z4\" layout=\"column\" flex=\"\"><md-toolbar md-theme=\"altTheme\" layout=\"row\" class=\"toolbar-border\"><div class=\"md-toolbar-tools\" flex=\"\">题目信息</div><md-button ng-click=\"vm.addTitle($event)\">添加</md-button></md-toolbar><div layout=\"column\"><md-input-container><label>题目</label> <input ng-model=\"vm.title.content\" type=\"text\" multiple=\"\"></md-input-container><md-input-container><label>描述</label> <input ng-model=\"vm.title.description\" type=\"text\" multiple=\"\"></md-input-container></div><div layout=\"column\"><div layout=\"row\"><h2 flex=\"\">答案</h2><md-button ng-click=\"vm.addAnswer($event)\">+</md-button></div><div ng-repeat=\"answer in vm.title.answers\"><div style=\"border: solid chartreuse 1px; margin: 5px;\"><md-input-container><label>答案</label> <input multiple=\"\" type=\"text\" ng-model=\"answer.content\"></md-input-container><md-checkbox ng-model=\"answer.correct\">是否为正确答案</md-checkbox></div></div></div></form></div>");
$templateCache.put("app/title/info/TitleInfo.html","<div layout=\"row\"><md-whiteframe class=\"md-whiteframe-z4\" layout=\"column\" flex=\"\"><md-toolbar md-theme=\"altTheme\" layout=\"row\" class=\"toolbar-border\"><div class=\"md-toolbar-tools\" flex=\"\">题目详情</div><md-button ng-click=\"\">添加学生</md-button></md-toolbar><div layout=\"column\"><div><span>题目:</span> <span>{{vm.title.content}}</span></div><div><span>描述:</span> <span>{{vm.title.description}}</span></div><div><h2>答案</h2><div ng-repeat=\"answer in vm.title.answers\" layout=\"row\" layout-align=\"center center\"><div>{{$index + 1}}</div><div flex=\"\">{{answer.content}}</div><div>{{answer.correct ? \"正确答案\" : \'\'}}</div></div><form name=\"vm.answerForm\"><div ng-repeat=\"answer in vm.answers\" layout=\"row\"><md-input-container flex=\"\"><label>答案</label> <input type=\"text\" required=\"\" ng-model=\"answer.content\"></md-input-container><md-checkbox ng-model=\"answer.correct\">正确答案</md-checkbox></div><div layout=\"row\"><label flex=\"\"></label><md-button ng-click=\"vm.addAnswer($event)\">添加答案</md-button><md-button ng-click=\"vm.save($event)\" ng-if=\"vm.answers.length > 0 && vm.answerForm.$valid\">保存</md-button></div></form></div></div></md-whiteframe></div>");
$templateCache.put("app/title/list/TitleList.html","<div layout=\"row\"><md-whiteframe class=\"md-whiteframe-z4\" layout=\"column\" flex=\"\"><md-toolbar md-theme=\"altTheme\" layout=\"row\" class=\"toolbar-border\"><div class=\"md-toolbar-tools\" flex=\"\">我的题目</div><md-button ng-href=\"/#/main/title/add\">添加</md-button></md-toolbar><div layout=\"row\" ng-repeat=\"title in vm.user.titles\"><lable>{{$index+1}}</lable><md-button flex=\"\" ng-href=\"/#/main/title/info?id={{title.id}}\">{{title.content}}</md-button><label flex=\"\">{{title.description}}</label></div><div ng-if=\"!vm.user.titles || vm.user.titles.length === 0\"><md-button ng-href=\"/#/main/title/add\">没有出过题目，点此添加</md-button></div></md-whiteframe></div>");
